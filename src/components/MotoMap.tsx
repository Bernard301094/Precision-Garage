...
import { X, MapPin, Trash2, ChevronDown, ChevronUp, StickyNote, AlertTriangle, Wrench } from 'lucide-react';

// ═══════════════════════════════════════════════════════════════════
// TIPOS DE DANO — categorias agrupadas
// ═══════════════════════════════════════════════════════════════════
...
// Mapa rápido: tipo → metadata da categoria
...
(Object.entries(DAMAGE_CATEGORIES) as [DamageCategory, { color: string; badge: string; items: readonly string[] }][])
  .forEach(([cat, meta]) => {
    meta.items.forEach(item => {
      DAMAGE_META[item] = { color: meta.color, badge: meta.badge, category: cat };
    });
  });
...
// ═══════════════════════════════════════════════════════════════════
// ZONAS DA MOTO (posições % sobre a imagem 16:9)
// ═══════════════════════════════════════════════════════════════════
const MOTO_ZONES = [
  { id: 'farol',        label: 'Farol Dianteiro',       cx: 13, cy: 42 },
  ...
  { id: 'guidao',       label: 'Guidão / Manoplas',     cx: 28, cy: 28 },
  { id: 'painel',       label: 'Painel / Instrumentos', cx: 35, cy: 32 },
  { id: 'tanque',       label: 'Tanque de Combustível', cx: 46, cy: 28 },
  { id: 'carenagem_l',  label: 'Carenagem Lateral',     cx: 50, cy: 55 },
  { id: 'motor',        label: 'Motor / Bloco',         cx: 50, cy: 68 },
  { id: 'banco',        label: 'Banco / Assento',       cx: 62, cy: 32 },
...
// URL da imagem da moto (mantida igual)
...
export const MotoMap: React.FC<MotoMapProps> = ({ pins, onChange }) => {
  ...
  // Posição onde o usuário tocou
  const [pendingPos, setPendingPos] = useState<{ x: number; y: number } | null>(null);

  // Formulário do novo pin — campos expandidos
  ...
  const confirmPin = () => {
    ...
    const severityMeta = SEVERITY_LEVELS.find(s => s.id === selSeverity)!;
    onChange([...pins, {
  ...
  return (
    <div className="space-y-4">
            ...
            <div key={pin.id} className="absolute z-10"
              ...
              >
                ...
                {parseInt(pin.severity) >= 3 && (
                  <span className="absolute -top-1 -right-1 w-3 h-3 rounded-full border border-black flex items-center justify-center"
                ...
                <AnimatePresence>
                  {activePin === pin.id && (
                    <motion.div
                      ...
                      className="absolute bottom-9 left-1/2 -translate-x-1/2 z-30 w-52 bg-[#111] border border-[#383838] rounded-2xl p-3 shadow-2xl text-left pointer-events-auto"
                    >
      ...
      {/* ══════════════════════════════════════════════════════════════════
          PAINEL DE DETALHES DO DANO — aparece ao tocar na moto
      ══════════════════════════════════════════════════════════════════ */}
      <AnimatePresence>
        {pendingPos && (
          <motion.div
          ...
          >
            ...
            <div className="px-4 py-4 space-y-5">
              ...
              <div className="space-y-2">
                ...
                <div className="grid grid-cols-4 gap-1.5">
                  {SEVERITY_LEVELS.map(sv => (
                    <button key={sv.id}
                      onClick={() => setSelSeverity(sv.id as SeverityId)}
                      className={`flex flex-col items-center py-2 px-1 rounded-xl border text-[9px] font-bold transition-all ${
                        selSeverity === sv.id
                          ? 'border-transparent scale-[1.05] shadow-lg'
                          : 'bg-[#0e0e0e] text-[#555] border-[#222]'
                      }`}
              ...
              <div className="space-y-2">
                ...
                <div className="flex flex-wrap gap-1.5">
                  {SIDE_OPTIONS.map(side => (
                    <button key={side}
                      onClick={() => setSelSide(selSide === side ? '' : side)}
                      className={`py-1.5 px-3 rounded-xl text-[10px] font-bold border transition-all ${
                        selSide === side
              ...
              <div className="space-y-2">
                ...
                />
                <textarea
                  placeholder="Observações adicionais (ex: cliente relatou queda há 3 meses...)"
                  value={notes}
              ...
              {/* ── Resumo rápido do dano configurado ── */}
              <div className="flex flex-wrap gap-1.5 items-center p-3 bg-[#0e0e0e] rounded-xl border border-[#1e1e1e]">
                ...
                <span className="text-[9px] px-2 py-0.5 rounded-md font-bold border" style={{
              ...
              <div className="flex gap-2 pt-1">
                <button
                  onClick={() => { setPendingPos(null); setCustomLabel(''); }}
                ...
                >✓ CONFIRMAR DANO #{nextNumber}</button>
              </div>
            </div>
          </motion.div>
        )}
                    ...
                    {isExpanded && (
                      <motion.div initial={{ height: 0, opacity: 0 }} animate={{ height: 'auto', opacity: 1 }} exit={{ height: 0, opacity: 0 }}
                        ...
                        <div className="px-4 pb-3 space-y-2.5 border-t border-[#1e1e1e] pt-3">
                          <div className="flex flex-wrap gap-1.5">
                            <span className="text-[9px] px-2 py-0.5 rounded-md font-bold border" style={{ backgroundColor: pinColor+'20', color: pinColor, borderColor: pinColor+'40' }}>{pin.category}</span>
